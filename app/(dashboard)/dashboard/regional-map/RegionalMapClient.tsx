...
const FALLBACK_HOSPITALS: OSMElement[] = [
  { id: "h1", lat: 17.2820, lon: 121.7990, tags: { name: "Tumauini Community Hospital", amenity: "hospital", "addr:street": "National Highway", "addr:city": "Tumauini" } },
  { id: "h2", lat: 17.1398, lon: 121.8885, tags: { name: "Gov. Faustino N. Dy, Sr. Memorial Hospital", amenity: "hospital", "addr:street": "Calamagui 2nd", "addr:city": "Ilagan City", phone: "+63-78-323-2009" } },
  { id: "h3", lat: 17.1352, lon: 121.8845, tags: { name: "Isabela Doctors General Hospital", amenity: "hospital", "addr:street": "National Highway", "addr:city": "Ilagan City", phone: "+63-78-624-0012" } },
  { id: "h4", lat: 16.9385, lon: 121.7772, tags: { name: "Cauayan District Hospital", amenity: "hospital", "addr:street": "Bucag", "addr:city": "Cauayan City" } }
];

const FALLBACK_POLICE: OSMElement[] = [
  { id: "p1", lat: 17.2680, lon: 121.8010, tags: { name: "Tumauini Police Station", amenity: "police", "addr:city": "Tumauini", phone: "117 / 911" } },
  { id: "p2", lat: 17.1430, lon: 121.8980, tags: { name: "Ilagan City Police Station", amenity: "police", "addr:city": "Ilagan City", phone: "+63-917-862-2345" } }
];

const FALLBACK_FIRE: OSMElement[] = [
  { id: "f1", lat: 17.2690, lon: 121.8150, tags: { name: "Tumauini Fire Station", amenity: "fire_station", "addr:city": "Tumauini", phone: "+63-78-323-0118" } },
  { id: "f2", lat: 17.1530, lon: 121.8950, tags: { name: "Ilagan Fire Station", amenity: "fire_station", "addr:city": "Ilagan City" } }
];
...
const createLabelIcon = (text: string) =>
  L.divIcon({
    ...
    html: `<div class="text-[9px] font-bold text-text-primary px-1.5 py-0.5 bg-black/75 border border-white/10 rounded backdrop-blur-sm shadow-md">${text}</div>`,
    iconAnchor: [0, 6],
  });
  ...
  L.divIcon({
    ...
    html: `
      <div class="flex items-center gap-1.5">
        ...
        <span class="text-[9px] font-bold text-white drop-shadow-[0_1px_2.5px_rgba(0,0,0,0.95)]">${name}</span>
      </div>
    `,
    iconSize: [120, 20],
    iconAnchor: [3, 10],
...
// --- Map coordinate tracker helper component ---
...
export default function RegionalMapClient() {
  ...
  const [mouseCoords, setMouseCoords] = useState<{ lat: number; lng: number }>({ lat: SITE_LAT, lng: SITE_LNG });
  ...
  useEffect(() => {
    ...
    ]).then(([boundary, riversData, roadsData, medicalData, govData, emergencyData]) => {
      if (!active) return;
  ...
  if (loading) {
    return (
      <div className="flex h-[750px] w-full items-center justify-center rounded-2xl border border-border-hairline bg-bg-panel/50 backdrop-blur-md">
  ...
  return (
    <div className="flex flex-col h-full w-full space-y-4">
              ...
              <div className="absolute top-10 left-0 w-full z-[1000] border border-white/10 bg-black/95 rounded-xl shadow-2xl p-1 max-h-48 overflow-y-auto backdrop-blur-md">
                {suggestions.map((m) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-text-muted hover:text-text-primary hover:bg-white/5 rounded-lg flex items-center justify-between group transition-all font-sans"
                  >
                    <span>{m.name}</span>
      ...
      {/* MAP VIEW CONTAINER */}
      <div className="relative w-full h-[650px] rounded-2xl overflow-hidden border border-border-hairline shadow-2xl bg-[#0B1418]">
        {/* MAP COMPONENT */}
        <MapContainer
        ...
        >
          ...
          {/* recoding map panning */}
          <MapRecenter center={mapCenter} zoom={mapZoom} />

          {/* Mouse coordinate tracking and map clicking */}
          <MapEventsTracker
            onMouseMove={(lat, lng) => setMouseCoords({ lat, lng })}
            onMapClick={() => {
              // Clicking empty map clears selections
          ...
          {layers.rivers &&
            rivers.map((way: OSMElement) => {
              ...
              const isPinacanauan = name.toLowerCase().includes("pinacanauan");
              ...
              return (
                <Polyline
                  ...
                  positions={positions}
                  pathOptions={{
                    color: isPinacanauan ? "#1FB6A6" : isStream ? "#2A4A5A" : "#3A6A8A",
                    weight: isPinacanauan ? 3.5 : isStream ? 1.2 : 2.2,
                    opacity: isPinacanauan ? 0.9 : 0.6,
                  }}
                  eventHandlers={{
                    click: (e) => {
                      ...
                      setSelectedItem({
                        ...
                        category: isPinacanauan ? "Major River System" : "Waterway / Stream",
                        address: "Isabela Province Watershed Area",
                        distance: `${dist.toFixed(2)} km`,
                        details: `Osm ID: ${way.id}\nPinacanauan River sat connection: ${isPinacanauan ? "YES" : "NO"}`,
                      });
                    },
                  }}
                />
          ...
          {/* LAYER 3 Special: Permanent labels for Pinacanauan River */}
          {layers.rivers &&
            rivers
              .filter((way: OSMElement) => way.tags?.name?.toLowerCase().includes("pinacanauan") && (way.geometry?.length || 0) > 4)
              .map((way: OSMElement, idx: number) => {
                ...
                return (
                  <Marker
                    ...
                    icon={createLabelIcon("Pinacanauan River")}
                    interactive={false}
                  />
                );
              })}
            ...
            roads.map((way: OSMElement) => {
              ...
              return (
                <Polyline
                  ...
                  positions={positions}
                  pathOptions={{
                    color: isMain ? "#8A7A4A" : "#6A5A3A",
                    weight: isMain ? 2.2 : 1.2,
                    opacity: 0.6,
                  }}
                  eventHandlers={{
                    click: (e) => {
          ...
          {layers.power && (
            <>
              {/* Dash lines between substations routed along Cagayan valley corridor */}
              <Polyline
                positions={[[17.6133, 121.7269], [17.4194, 121.7733], [17.2736, 121.8078], [17.15, 121.875]]} // Tuguegarao - Cabagan - Tumauini - Ilagan
                pathOptions={{ color: "#E8A33D", dashArray: "6 6", weight: 2, opacity: 0.7 }}
              />
              <Polyline
                positions={[[17.15, 121.875], [16.92, 121.77]]} // Ilagan - Cauayan
                pathOptions={{ color: "#E8A33D", dashArray: "6 6", weight: 2, opacity: 0.7 }}
              />
              <Polyline
                positions={[[16.92, 121.77], [16.8833, 121.5833]]} // Cauayan - San Mateo
                pathOptions={{ color: "#E8A33D", dashArray: "6 6", weight: 2, opacity: 0.7 }}
              />
              {/* Plant to closest grid hub (Tumauini Center Hub) */}
              <Polyline
                positions={[[SITE_LAT, SITE_LNG], [17.2736, 121.8078]]} // Tumauini HEPP - Tumauini Grid Interface
                pathOptions={{ color: "#E8A33D", dashArray: "6 6", weight: 2.2, opacity: 0.8 }}
                eventHandlers={{
                  click: (e) => {
                    L.DomEvent.stopPropagation(e);
                    setSelectedItem({
                      name: "HEPP Interconnection Line",
                      category: "Power Transmission",
                      address: "Tumauini HEPP to Tumauini Grid Hub",
                      distance: `${turf.distance(fromPoint, turf.point([121.8078, 17.2736]), { units: "kilometers" }).toFixed(2)} km`,
                      details: "132kV Transmission Interconnection Line linking Tumauini plant generation with the main Cagayan Valley grid corridor.",
                    });
                  },
                }}
              />
                  ...
                  eventHandlers={{
                    click: (e) => {
                      ...
                      setSelectedItem({
                        ...
                        distance: `${dist.toFixed(2)} km`,
                        details: "NGCP — National Grid Corporation of the Philippines. Critical transmission node in operations area.",
                      });
                    },
                  }}
                />
          ...
          {layers.medical &&
            emergencyLists.allHospitals.map((node) => (
              <Marker
                key={node.id}
                position={[node.lat ?? 0, (node.lon ?? node.lng ?? 0)]}
          ...
          {layers.government &&
            government.map((node: OSMElement) => (
              <Marker
                key={node.id}
          ...
          {layers.emergency &&
            [...emergencyLists.allPolice, ...emergencyLists.allFire].map((node) => {
        ...
        {layers.emergency && (
          <div
            className={cn(
              "absolute top-4 left-4 z-[999] w-80 border border-white/10 bg-black/85 backdrop-blur-md rounded-2xl shadow-2xl transition-all duration-300 overflow-hidden font-sans",
        ...
        <div className="absolute bottom-4 left-4 z-[999] w-80 border border-white/10 bg-black/90 backdrop-blur-md rounded-2xl shadow-2xl p-4 font-sans text-xs">
          {selectedItem ? (
        ...
        <div className="absolute top-4 right-4 z-[999] max-w-sm border border-white/10 bg-black/85 backdrop-blur-md rounded-2xl shadow-2xl p-4 font-sans">
          ...
          <div className="grid grid-cols-2 gap-2 text-[10px]">
            ...
            >
              ...
              <span>Hospitals ({emergencyLists.counts.hospitals})</span>
            </button>
            ...
            >
              ...
              <span>Emergency ({emergencyLists.counts.police + emergencyLists.counts.fire})</span>
            </button>
          </div>
        </div>

        {/* FLOATING MOUSE COORDINATES BAR (BOTTOM CENTER) */}
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[999] px-4 py-1.5 bg-black/80 border border-white/10 rounded-full shadow-lg backdrop-blur-sm text-[10px] text-text-muted font-mono tracking-wider flex items-center gap-3">
          <span className="flex items-center gap-1">
            <span className="text-flow-teal font-sans">LAT:</span> {mouseCoords.lat.toFixed(5)}
          </span>
          <span className="text-white/10">|</span>
          <span className="flex items-center gap-1">
            <span className="text-flow-teal font-sans">LON:</span> {mouseCoords.lng.toFixed(5)}
          </span>
        </div>
      </div>
    </div>
